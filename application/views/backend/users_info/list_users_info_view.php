<div class="page-wrapper">
    <div class="page-content">
        <!--breadcrumb-->
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">User Info</div>
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">

                        <li class="breadcrumb-item"><a href="javascript:;"><i class="bx bx-home-alt"></i></a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">List User</li>
                    </ol>
                </nav>
            </div>

        </div>
        <!--end breadcrumb-->

        <div class="card">
            <div class="card-body">
                <div class="d-lg-flex align-items-center mb-4 gap-3">
                <form class="row g-3" action="<?=base_url();?>auth/is_session/users_info/user_list/" method="GET"
                        enctype="multipart/form-data">
                        <div class="col-md-1">
                            <label for="inputFirstName" class="form-label">Show</label>
                            <select name="show" class="form-control">
                                <!-- <option value="5"
                                    <?php echo ($this->input->get('show', true) == '5') ? 'selected' : ''; ?>>5
                                </option> -->
                                <option value="10"
                                    <?php echo ($this->input->get('show', true) == '10') ? 'selected' : ''; ?>>
                                    10
                                </option>
                                <option value="30"
                                    <?php echo ($this->input->get('show', true) == '30') ? 'selected' : ''; ?>>30
                                </option>
                                <option value="60"
                                    <?php echo ($this->input->get('show', true) == '60') ? 'selected' : ''; ?>>60
                                </option>
                                <option value="100"
                                    <?php echo ($this->input->get('show', true) == '100') ? 'selected' : ''; ?>>100
                                </option>
                                <option value="200"
                                    <?php echo ($this->input->get('show', true) == '200') ? 'selected' : ''; ?>>200
                                </option>
                            </select>
                        </div>
                        <div class="col-md-3">
                            <label for="inputFirstName" class="form-label">&nbsp;&nbsp;</label>
                            <input type="text" class="form-control" name="q"
                                value="<?php echo html_escape($this->input->get('q', true)); ?>"
                                placeholder="Search User phone" id="inputFirstName">
                        </div>
                        <div class="col-md-3">
                            <label for="inputFirstName" class="form-label">&nbsp;&nbsp;</label>
                            <input type="date" class="form-control" name="Fdate"
                                value="<?php echo html_escape($this->input->get('Fdate', true)); ?>"
                                placeholder="Search User Phone" id="inputFirstName">
                        </div>
                        <div class="col-md-3">
                            <label for="inputFirstName" class="form-label">&nbsp;&nbsp;</label>
                            <input type="date" class="form-control" name="Tdate"
                                value="<?php echo html_escape($this->input->get('Tdate', true)); ?>"
                                placeholder="Search User Phone" id="inputFirstName">
                        </div>
                        <div class="col-md-2">
                            <label for="inputFirstName" class="form-label">&nbsp;&nbsp;&nbsp;&nbsp;</label>
                            <button type="submit" name="submit" class="btn btn-primary" style="margin-bottom: -68px;">Filter</button>
                        </div>
                        <?php echo form_close(); ?>
                        <button  onclick="ExportData()" type="submit" class="btn btn-primary" style="margin-left: 14px;margin-top: 26px;"><i class="bx bx-down-arrow-alt"></i>Export</button>
                        <div class="ms-auto"><a href="<?=base_url()?>auth/is_session/users_info/user_list_add/"
                                    class="btn btn-primary radius-30 mt-2 mt-lg-0" style="margin-bottom: -30px;"><i class="bx bxs-plus-square"></i>Add User</a></div>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" id="tablelist">
                        <thead class="table-light">
                            <tr>
                                <th>S No</th>
                                <th>Name</th>
                                <th>Email</th>
                                <th>Phone</th>
                                <th>Image</th>
                                <th>View Details</th>
                                <th>Date</th>
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>

                            <?php 
                            if(!empty($user_list)){
                  foreach($user_list as $key =>$row)
                  {
                  ?><tr id="sortable">
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div>
                                        </div>
                                        <div class="ms-2">
                                            <h6 class="mb-0 font-14"><?= $start; ?></h6>
                                        </div>
                                    </div>
                                </td>
                                <td><?= $row->name?> </td>
                                <td><?= $row->email?> </td>
                                <td><?= $row->phone?> </td>
                                <?php if(!empty($row->image)){ ?>
                                <td><img src="<?=base_url()?><?= $row->image?>" style="width: 74px;height: 55px;"> </td>
                                <?php }else{ ?>
                                <td><img src="<?=base_url()?>uploads/profile/dummy.png" style="width: 74px;height: 55px;"> </td>
                                <?php } ?>
                                <td><a href="<?=base_url()?>auth/is_session/users_info/user_list_view/?jwt_token=<?php echo encrypt_decrypt($row->id, 'encrypt')?>/" name="submit" value="add_user" class="btn btn-primary"><i class="bx bxs-show" aria-hidden="true"></i></a></td>
                                <td><?php echo Dateconversion($row->created_at); ?></td>
                                <td>
                                    <div class="d-flex order-actions">
                                        <a href="<?=base_url()?>auth/is_session/users_info/user_list_edit/?jwt_token=<?php echo encrypt_decrypt($row->id, 'encrypt')?>/"
                                            class=""><i class='bx bxs-edit'
                                                style="font-size: 25px;color: #008000;"></i></a>
                                        <a href="#"
                                            onclick="validate('<?php echo $row->id;?>','<?php echo $row->name;?>')"
                                            class="ms-3"><i class='bx bxs-trash'
                                                style="font-size: 25px;color: #ef0716;"></i></a>
                                    </div>
                                    
                                    <form method="post"
                                        action="<?=site_url('/auth/is_session/users_info/user_showpay/');?>">
                                        <input type="hidden" name="idname" value="<?=$row->id;?>">
                                        <input onChange="this.form.submit()" type="checkbox" name="feature"
                                            id="status_a<?=$row->id;?>" class="check"
                                            value="<?php if($row->status==0){echo '1';}else{echo '0';}?>"
                                            <?php if($row->status==1)echo 'checked';?>>
                                        <label for="status_a<?=$row->id;?>" class="checktoggle">checkbox</label>
                                    </form>
                                </td>
                            </tr>
                            
                            
                            <?php
                            $start++;
                }
              ?>
<?php }else{ ?>
                            <td colspan="9" style="text-align: center;color: red;font-size: 19px;">No Record Found</td>

                            <?php } ?>
                        </tbody>

                    </table>
                    <nav aria-label="Page navigation example">

                        <ul class="pagination">

                            <?php echo $this->pagination->create_links(); ?>

                        </ul>

                    </nav>
                </div>
                
                
                
            </div>
        </div>


    </div>
</div>

<script>
$(document).ready(function() {

    // Initialize sortable
    $("#sortable").sortable();

    // Save order
    $('#submit').click(function() {
        var imageids_arr = [];
        // get image ids order
        $('#sortable li').each(function() {
            var id = $(this).data('id');
            i imageids_arr.push(id);
        });

        // AJAX request
        $.ajax({
            url: 'ajaxfile.php',
            type: 'post',
            data: {
                imageids: imageids_arr
            },
            success: function(response) {
                if (response == 1)
                    alert('Save successfully.');
            }
        });
    });
});
</script>
<script>
  $(function() {
    TablesDatatables.init();
  });
  function validate(id, title) {
    swal({
      title: "Are you sure?",
      text: "Are you sure you want to delete this record" + " " + title,
      type: "warning",
      showCancelButton: true,
      confirmButtonColor: "#DD6B55",
      confirmButtonText: "Yes, Delete it!",
      closeOnConfirm: false
    }, function() {
      //swal("Deleted!", "Your record has been deleted successfully.", "success");
      $(location).attr('href', '<?php echo base_url()?>auth/is_session/users_info/delete_user/' +id);
    });
  }
</script>
<script>
    function ExportData(){
        window.location.href='<?=base_url()?>auth/is_session/users_info/user_list_history_download/?show='+$("[name=show]").val()+'&q='+$("[name=q]").val()+'&Fdate='+$("[name=Fdate]").val()+'&Tdate='+$("[name=Tdate]").val()+'';
    }
</script>