    ...
    <div class="page-content">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <form class="row g-3" action="<?=base_url();?>auth/is_session/count_statistics/list_count_statistics/<?=$this->uri->segment(5)?>" method="GET"
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" name="q"
                                ...
                                placeholder="Search <?=$td_modules['module_name']?>" id="inputFirstName">
                        </div>
                        <div class="col-md-3">
                            <label for="inputFirstName" class="form-label">&nbsp;&nbsp;</label>
                    ...
                    <!--<div class="ms-auto"><a href="<?=base_url()?>auth/is_session/count_statistics/add_count_statistics/<?=$this->uri->segment(5)?>/"-->
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" id="tablelist">
                        ...
                        <tbody>
                            ...
                            <?php 
                            if(!empty($td_count_statistics)){
                  foreach(array_reverse($td_count_statistics) as $key =>$row)
                  {
                  ?><tr id="sortable">
                                <td>
                                ...
                                <td><?= $row->counter_icon?> </td>
                                <td><?= $row->counter_timer?> </td>
                                <td><?= $row->counter_operator?> </td>
                                <td><?= $row->counter_title?> </td>
                                <td><?php echo Dateconversion($row->created_at); ?></td>
                                <td><?php echo Timeconversion($row->created_at); ?></td>
                                <td>
                                    <form method="post"
                                        action="<?=site_url('/auth/is_session/count_statistics/showpay');?>">
                                ...
                                <td>
                                    <div class="d-flex order-actions">
                                        <a href="<?=base_url()?>auth/is_session/count_statistics/edit_count_statistics/<?=$this->uri->segment(5)?>/?jwt_token=<?php echo encrypt_decrypt($row->id, 'encrypt')?>"
                                            class=""><i class='bx bxs-edit' style="font-size: 25px;color: #008000;"></i></a>
                                        <a href="#" onclick="validate('<?php echo $row->id;?>','<?php echo $row->counter_title;?>')" class="ms-3"><i class='bx bxs-trash' style="font-size: 25px;color: #ef0716;"></i></a>
                                    </div>
                                </td>
                            </tr>
                            <?php
...
<script>
  ...
  function validate(id, title) {
    swal({
      ...
      $(location).attr('href', '<?php echo base_url()?>auth/is_session/count_statistics/delete_count_statistics/' +id);
    });
  }
</script>