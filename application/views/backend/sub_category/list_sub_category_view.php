<div class="page-wrapper">
    <div class="page-content">
        <!--breadcrumb-->
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">Sub Category</div>
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        <li class="breadcrumb-item"><a href="javascript:;"><i class="bx bx-home-alt bx-color"></i></a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">List Sub Category </li>
                    </ol>
                </nav>
            </div>

        </div>
        <!--end breadcrumb-->

        <div class="card">
            <div class="card-body">
                <div class="d-lg-flex align-items-center mb-4 gap-3">
                    <form class="row g-3" action="<?=base_url();?>auth/is_session/sub_category/list_sub_category/"
                        method="GET" enctype="multipart/form-data">
                        <div class="col-md-2">
                            <label for="inputFirstName" class="form-label">Show</label>
                            <select name="show" class="form-control">
                                <option value="10"
                                    <?php echo ($this->input->get('show', true) == '10') ? 'selected' : ''; ?>>
                                    10
                                </option>
                                <option value="30"
                                    <?php echo ($this->input->get('show', true) == '30') ? 'selected' : ''; ?>>30
                                </option>
                                <option value="60"
                                    <?php echo ($this->input->get('show', true) == '60') ? 'selected' : ''; ?>>60
                                </option>
                                <option value="100"
                                    <?php echo ($this->input->get('show', true) == '100') ? 'selected' : ''; ?>>100
                                </option>
                                <option value="200"
                                    <?php echo ($this->input->get('show', true) == '200') ? 'selected' : ''; ?>>200
                                </option>
                            </select>
                        </div>
                        <div class="col-md-3">
                            <label for="inputFirstName" class="form-label">&nbsp;&nbsp;</label>
                            <?php 
                            $where = array("status" => "1","pro_status" => "1");
                            $result_array = array("result_array");
                            $category = $this->Main_model->get_data($where, "td_category",null,null,null,null, $result_array);
                            ?>
                            <select class="form-control" name="category"
                                value="<?php echo html_escape($this->input->get('category', true)); ?>">
                                <option value="">Select Category</option>
                                <?php foreach($category as $key => $val){ ?>
                                    <option value="<?=$val['category_id']?>" <?php if($this->input->get('category', true) == $val['category_id']){echo 'Selected';}?>><?=$val['category']?></option>
                                    <?php } ?>
                            </select>
                        </div>
                        <div class="col-md-3">
                            <label for="inputFirstName" class="form-label">&nbsp;&nbsp;</label>
                            <input type="text" class="form-control" name="q"
                                value="<?php echo html_escape($this->input->get('q', true)); ?>"
                                placeholder="Search Sub Category Name" id="inputFirstName">
                        </div>
                        <div class="col-md-3">
                            <label for="inputFirstName" class="form-label">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
                            <button type="submit" name="submit" class="btn btn-primary px-5">Filter</button>
                        </div>
                        <?php echo form_close(); ?>

                        <div class="ms-auto"><a href="<?=base_url()?>auth/is_session/sub_category/add_sub_category/"
                                class="btn btn-primary radius-30 mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i>Add New
                                Sub Category</a></div>
                </div>
                <div class="table-responsive">
                    <table class="table mb-0">
                        <thead class="table-light">
                            <tr>
                                <th>S No</th>
                                <th>Category</th>
                                <th>Sub Category</th>

                                <th>Date</th>
                                <th>Time</th>
                                <th>Status</th>
                                <th>Show Menu</th>
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <?php  
                                if(!empty($td_sub_category)){
                                foreach($td_sub_category as $key =>$row){ ?>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div>
                                        </div>
                                        <div class="ms-2">
                                            <h6 class="mb-0 font-14"><?= $start; ?></h6>
                                        </div>
                                    </div>
                                </td>
                                <td><?= $row->category?> </td>
                                <td><?= $row->sub_category?> </td>

                                <td><?php echo Dateconversion($row->created_at); ?></td>
                                <td><?php echo Timeconversion($row->created_at); ?></td>
                                <td>
                                    <form method="post"
                                        action="<?=site_url('/auth/is_session/sub_category/list_sub_category/showpay');?>">
                                        <input type="hidden" name="idname" value="<?=$row->id;?>">
                                        <input onChange="this.form.submit()" type="checkbox" name="feature"
                                            id="status_a<?=$row->id;?>" class="check"
                                            value="<?php if($row->pro_status==0){echo '1';}else{echo '0';}?>"
                                            <?php if($row->pro_status==1)echo 'checked';?>>
                                        <label for="status_a<?=$row->id;?>" class="checktoggle">checkbox</label>
                                    </form>
                                </td>
                                <td>
                                    <form method="post"
                                        action="<?=site_url('/auth/is_session/sub_category/list_sub_category/menu_showpay');?>">
                                        <input type="hidden" name="show_menu" value="<?=$row->id;?>">
                                        <input onChange="this.form.submit()" type="checkbox" name="feature_id"
                                            id="menu_showpay_a<?=$row->id;?>" class="check"
                                            value="<?php if($row->show_menu==0){echo '1';}else{echo '0';}?>"
                                            <?php if($row->show_menu==1)echo 'checked';?>>
                                        <label for="menu_showpay_a<?=$row->id;?>"
                                            class="checktoggle">checkbox</label>
                                    </form>
                                </td>
                                <td>
                                    <div class="d-flex order-actions">
                                        <a href="<?=base_url()?>auth/is_session/sub_category/edit_sub_category/index?jwt_token=<?php echo encrypt_decrypt($row->id, 'encrypt')?>"
                                            class=""><i class='bx bxs-edit'
                                                style="font-size: 25px;color: #008000;"></i></a>
                                        <a href="#"
                                            onclick="validate('<?php echo $row->id;?>','<?php echo $row->sub_category;?>')"
                                            class="ms-3" style="font-size: 25px;color: #ef0716;"><i
                                                class='bx bxs-trash'></i></a>
                                    </div>
                                </td>
                            </tr>
                            <?php
                            $start++;
                }
              ?>
              <?php }else{ ?>
                <td colspan="7" style="text-align: center;color: red;font-size: 19px;">No Record Found</td>
                
              <?php } ?>
                        </tbody>
                    </table>
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            <?php echo $this->pagination->create_links(); ?>
                        </ul>
                    </nav>
                </div>
            </div>
        </div>


    </div>
</div>

<script>
$(function() {
    TablesDatatables.init();
});

function validate(id, title) {
    swal({
        title: "Are you sure?",
        text: "Are you sure you want to delete this record" + " " + title,
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#DD6B55",
        confirmButtonText: "Yes, Delete it!",
        closeOnConfirm: false
    }, function() {
        //swal("Deleted!", "Your record has been deleted successfully.", "success");
        $(location).attr('href',
            '<?php echo base_url()?>auth/is_session/sub_category/list_sub_category/delete_sub_category/' + id);
    });
}
</script>