    ...
    <div class="page-content">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <?php if(!empty($_GET['portfolio_id'])){ ?>
                <form class="row g-3" action="<?=base_url();?>auth/is_session/faqs/faqs_list/<?=$this->uri->segment(5)?>/?portfolio_id=<?=$_GET['portfolio_id']?>" method="GET" enctype="multipart/form-data">
                     <input type="hidden" class="form-control" name="portfolio_id" value="<?php echo $_GET['portfolio_id'];?>" placeholder="Search Faqs Question" id="inputFirstName">
                    <?php }else{ ?>
                    <form class="row g-3" action="<?=base_url();?>auth/is_session/faqs/faqs_list/<?=$this->uri->segment(5)?>" method="GET" enctype="multipart/form-data">
                    <?php } ?>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" name="q"
                                ...
                                placeholder="Search Faqs Question" id="inputFirstName">
                        </div>
                        <div class="col-md-3">
                            <label for="inputFirstName" class="form-label">&nbsp;&nbsp;</label>
                       ...
                       <?php if(!empty($_GET['portfolio_id'])){ ?>
                    <div class="ms-auto"><a href="<?=base_url()?>auth/is_session/faqs/add_faq/<?=$this->uri->segment(5)?>/?portfolio_id=<?=$_GET['portfolio_id'];?>/"
                            class="btn btn-primary radius-30 mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i>Add New <?=$td_modules['module_name']?></a></div>
                            <?php }else{ ?>
                            <div class="ms-auto"><a href="<?=base_url()?>auth/is_session/faqs/add_faq/<?=$this->uri->segment(5)?>/"
                            class="btn btn-primary radius-30 mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i>Add New <?=$td_modules['module_name']?></a></div>
                            <?php } ?>
                </div>
                <div class="table-responsive">
                  ...
                  ?><tr id="sortable">
                                ...
                                <td>
                                    <form method="post"
                                        action="<?=site_url('/auth/is_session/faqs/showpay');?>">
                                ...
                                <td>
                                    <div class="d-flex order-actions">
                                        <a href="<?=base_url()?>auth/is_session/faqs/edit_faq/<?=$this->uri->segment(5)?>/?jwt_token=<?php echo encrypt_decrypt($row->id, 'encrypt')?>"
                                            class=""><i class='bx bxs-edit' style="font-size: 25px;color: #008000;"></i></a>
                                        <a href="#" onclick="validate('<?php echo $row->id;?>','<?php echo $row->question;?>')" class="ms-3"><i class='bx bxs-trash' style="font-size: 25px;color: #ef0716;"></i></a>
                                    </div>
                                </td>
                            </tr>
                            <?php
...
<script>
  ...
  function validate(id, title) {
    swal({
      ...
      $(location).attr('href', '<?php echo base_url()?>auth/is_session/faqs/delete_faq/' +id);
    });
  }
</script>