...
class Add_articles extends My_Controller {
	public function __construct(){
		parent::__construct();
	}
	public function index(){  
		if($_POST){
			if($_POST["submit"] == "td_articles"){
				...
				if (is_uploaded_file($_FILES['articles_image']['tmp_name'])) {
                    ...
                    if (!$this->upload->do_upload('articles_image', FALSE) && is_uploaded_file($_FILES['articles_image']['tmp_name'])) {
                        ...
                        redirect(base_url() . 'auth/is_session/articles/add_articles/');
                    } else {
                        $out = $this->upload->data();
                        $data['articles_image'] = 'uploads/articles/' . $out['orig_name'];
                    }
                }
				$this->Main_model->insert_data($data, "td_articles");
        ...
        $this->data['_view_'] = 'backend/articles/add_articles_view';
