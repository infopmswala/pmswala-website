    ...
    <div class="page-content">
            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">List SEO Pages</li>
                    </ol>
                </nav>
            </div>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <form class="row g-3" action="<?=base_url();?>auth/is_session/seo/list_seo/" method="GET"
                        ...
                        <?php echo form_close(); ?>
                    <div class="ms-auto"><a href="<?=base_url()?>auth/is_session/seo/add_seo/"
                ...
                <div class="table-responsive">
                    <table class="table mb-0">
                      ...
                      <tbody>
                                ...
                                <?php 
                                if(!empty($td_seo)){
                  foreach($td_seo as $key =>$row)
                  {
                  ?> <tr>
                                    <td><?= $start; ?></td>
                                    <td><?= $row->page_name?> </td>
                                    <td><?= $row->meta_tag_title?> </td>
                                   <!-- <td><?= $row->meta_tag_keywords?> </td>
                                   -->
                                    <td><?php echo Dateconversion($row->created_at); ?></td>
                                    <td>
                                       <form method="post"
                                           action="<?=site_url('/auth/is_session/seo/list_seo/showpay');?>">
                                    ...
                                    <td>
                                        <div class="d-flex order-actions">
                                            <a href="<?=base_url()?>auth/is_session/seo/edit_seo/index?jwt_token=<?php echo encrypt_decrypt($row->id, 'encrypt')?>"
                                                class=""><i class='bx bxs-edit' style="font-size: 25px;color: #008000;"></i></a>
                                            <a href="javascript:;" class="ms-3" onclick="validate('<?php echo $row->id;?>','<?php echo $row->meta_tag_title;?>')"><i class='bx bxs-trash' style="font-size: 25px;color: #ef0716;"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                <?php
...
<script>
  ...
  function validate(id, category) {
    swal({
      ...
      $(location).attr('href', '<?php echo base_url()?>auth/is_session/seo/list_seo/delete_seo/' +id);
    });
  }
</script>