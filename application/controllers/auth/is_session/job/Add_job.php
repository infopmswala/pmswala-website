...
class Add_job extends My_Controller {
	public function __construct(){
		parent::__construct();
	}
	public function index(){  
		if($_POST){
			if($_POST["submit"] == "td_job"){
				...
				if (is_uploaded_file($_FILES['job_image']['tmp_name'])) {
                    ...
                    if (!$this->upload->do_upload('job_image', FALSE) && is_uploaded_file($_FILES['job_image']['tmp_name'])) {
                        ...
                        redirect(base_url() . 'auth/is_session/job/add_job/');
                    } else {
                        $out = $this->upload->data();
                        $data['job_image'] = 'uploads/job/' . $out['orig_name'];
                    }
                }
				$this->Main_model->insert_data($data, "td_job");
        ...
        $this->data['_view_'] = 'backend/job/add_job_view';
