            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">List Fund Details</li>
                    </ol>
                </nav>
            </div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-lg-flex align-items-center mb-4 gap-3">
                    <form class="row g-3" action="<?=base_url();?>auth/is_session/portfolio/list_fund_details/<?=$td_modules['module_id']?>/?portfolio_id=<?php echo $_GET['portfolio_id'];?>" method="GET"
                        enctype="multipart/form-data">
                        <input type="hidden" class="form-control" name="portfolio_id" value="<?php echo $_GET['portfolio_id']; ?>" placeholder="Search Portfolio Name" id="inputFirstName">
                        <div class="col-md-3">
                        ...
                        <?php echo form_close(); ?>
                                                <div class="ms-auto"><a href="<?=base_url()?>auth/is_session/portfolio/add_fund_details/<?=$td_modules['module_id']?>/?portfolio_id=<?php echo $_GET['portfolio_id'];?>"
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody class="row_position">
                            <?php 
                            if(!empty($td_fund_details)){
                            foreach($td_fund_details as $key =>$row){ ?>
                            <tr id="<?=$row->id?>">
                                <td>
                                ...
                                <td><?= $row->fund_details_title?> </td>
                                <td><?= $row->fund_details_percentage?> </td>
                                <td>
                                    <form method="post"
                                        action="<?=site_url('/auth/is_session/portfolio/fund_details_showpay');?>">
                                ...
                                <td>
                                    <div class="d-flex order-actions">
                                        <a href="<?=base_url()?>auth/is_session/portfolio/edit_fund_details/<?=$td_modules['module_id']?>/?jwt_fund_details=<?php echo encrypt_decrypt($row->id, 'encrypt')?>"
                                            class=""><i class='bx bxs-edit'
                                                style="font-size: 25px;color: #008000;"></i></a>
                                        <a href="#"
                                            onclick="validate('<?php echo $row->id;?>','<?php echo $row->fund_details_title;?>')"
...
function validate(id, title) {
    swal({
        ...
        $(location).attr('href', '<?php echo base_url()?>auth/is_session/portfolio/delete_fund_details/' +
            id);
    });
}
</script>
