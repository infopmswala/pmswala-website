<div class="page-wrapper">
    <div class="page-content">
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">Service Modules</div>
            <div class="ps-3">
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-lg-flex align-items-center mb-4 gap-3">
                    <form class="row g-3" action="<?=base_url();?>auth/is_session/service/service_list/<?=$td_modules['module_id']?>/" method="GET"
                        ...
                        <?php echo form_close(); ?>
                        <div class="ms-auto"><a href="<?=base_url()?>auth/is_session/service/service_add/<?=$td_modules['module_id']?>/"
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            <?php 
                             if(!empty($td_services)){
                  foreach($td_services as $key =>$row)
                  {
                  ?><tr id="sortable">
                                <td>
                                ...
                                <td><?php if($row->service_image){ ?><img src="<?=base_url()?><?= $row->service_image?>"
                                        alt="<?= $row->service_title?>"
                                        style="height:60px; width:100px;"><?php }else{?><?=no_image(); } ?></td>

                                <td><?php echo Dateconversion($row->created_at); ?></td>
                                <td>
                                    <form method="post"
                                        action="<?=site_url('/auth/is_session/service/showpay/'.$td_modules['module_id'].'/');?>">
                                ...
                                <td>
                                    <div class="d-flex order-actions">
                                        <a href="<?=base_url()?>auth/is_session/service/edit/<?=$td_modules['module_id']?>/?jwt_token=<?php echo encrypt_decrypt($row->id, 'encrypt')?>/"
                                            class=""><i class='bx bxs-edit'
                                                style="font-size: 25px;color: #008000;"></i></a>
                                        <a href="#"
                                            onclick="validate('<?php echo $row->id;?>','<?php echo $row->service_title;?>')"
...
function validate(id, title) {
    swal({
        ...
        $(location).attr('href', '<?php echo base_url()?>auth/is_session/service/delete_service/' +
            id);
    });
}
</script>
